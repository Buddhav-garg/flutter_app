// Money is always an int in piase inside Bankease App 
// These helpers are the only place we convert to and from rupees
// 1234567890 -> '₹ 1,23,45,678.90'

String formatRupees(int paise, {bool showSymbol = false}) {
...
/// parse the user input such as '49.8' or '1000' into paise without using doubles

int? parseRupeesToPaise(String input){
  final text = input.trim().replaceAll(',','');
  if(!RegExp(r'^\d{0,9}(.\d{1,2})?$').hasMatch(text)) return null;
